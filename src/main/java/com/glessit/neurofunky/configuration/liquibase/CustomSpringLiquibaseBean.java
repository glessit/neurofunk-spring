...
public class CustomSpringLiquibaseBean implements InitializingBean, BeanNameAware, ResourceLoaderAware {
        ...
        protected void init() {
            ...
            try {
                ...
                if (resources.length == 0 || resources.length == 1 && !resources[0].exists()) { //sometimes not able to look up by empty string, try all the liquibase packages
                    ...
                    int i = 0;
                    for (Resource manifest : ResourcePatternUtils.getResourcePatternResolver(getResourceLoader()).getResources(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX + "META-INF/MANIFEST.MF")) {
                        if (manifest.exists() && i==0) {
                            i++;
                            InputStream inputStream = null;
                            try {
                                inputStream = manifest.getInputStream();
                                Manifest manifestObj = new Manifest(inputStream);
                                String[] packages = manifestObj.getMainAttributes().getValue("Liquibase-Package").split("\\s*,\\s*");
                                for (String fullPackage : packages) {
                                    liquibasePackages.add(fullPackage);
                                }
                            } finally {
                                if (inputStream != null) {
                                    inputStream.close();
                    ...
                    for (String foundPackage : liquibasePackages) {
                        resources = ResourcePatternUtils.getResourcePatternResolver(getResourceLoader()).getResources(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX + foundPackage.replaceAll("\\.","/"));
    ...
    protected final Logger log = LogFactory.getLogger(CustomSpringLiquibaseBean.class.getName());
     ...
     * </code>
     *
     * And {@link CustomSpringLiquibaseBean} is configured like:
     * <code>
    ...
    public CustomSpringLiquibaseBean() {
        super();
    }
    ...
    public void afterPropertiesSet() throws LiquibaseException {
        ConfigurationProperty shouldRunProperty = liquibase.configuration.LiquibaseConfiguration.getInstance().getProperty(GlobalConfiguration.class, GlobalConfiguration.SHOULD_RUN);

        if (!shouldRunProperty.getValue(Boolean.class)) {
            LogFactory.getLogger().info("Liquibase did not run because "+ liquibase.configuration.LiquibaseConfiguration.getInstance().describeValueLookupLogic(shouldRunProperty)+" was set to false");
            return;
        }
        if (!shouldRun) {
